...
public class CreatePost extends base {
	...
	@Test(priority = 1)
	public void CreatelearningPostwithYoutubeLink() throws IOException, InterruptedException {
		MethodName = new Throwable().getStackTrace()[0].getMethodName();
		log.info("Initiating Method :' " + MethodName + " '");
		w.until(ExpectedConditions.elementToBeClickable(hp.getLearningGridButton()));
		hp.getLearningGridButton().click();
								...
								try {
									...
									if (cpp.getConfirmationMessage().getText()
										...
										try {
											...
											Assert.assertTrue(false,
													"Data mismatched under My Pending Post Section" + e);
	...
	public void DeclineDraft() throws IOException, InterruptedException {
		MethodName = new Throwable().getStackTrace()[0].getMethodName();
		log.info("Initiating Method :' " + MethodName + " '");
		try {
